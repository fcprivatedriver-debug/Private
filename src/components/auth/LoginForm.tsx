...
function LoginFormInner() {
  ...
  // Already signed in → leave login without trapping the form in a spinner
  useEffect(() => {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res === null) {
        setError(
          "O login demorou demasiado. Verifique a ligação e tente novamente.",
        );
        setLoading(false);
        return;
      }
      ...
      go(role);
      // Keep loading=true until navigation completes; reset if navigate is blocked
      window.setTimeout(() => {
  ...
  // Show the real form immediately — never block the page on session hydration.
  // If already authenticated, a short status line appears above the form while redirecting.
      ...
      fallback={
        <section className="auth-shell fade-up">
          <div className="container" style={{ maxWidth: 440 }}>
            ...
            <p className="page-lead">Aceda à sua conta ZELU.</p>
