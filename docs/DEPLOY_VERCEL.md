# Deploy Nina → Vercel / addandknow.pt

Nina usa **PostgreSQL** (Neon, schema `nina`) + Auth.js + Resend.

## Produção

- Domínio: **https://addandknow.pt**
...
1. Settings → Domains → adicionar `addandknow.pt` (primary)
2. Adicionar `www.addandknow.pt` com redirect → `addandknow.pt`
...
## Resend (`no-reply@addandknow.pt`)

1. [Resend → Domains](https://resend.com/domains) → Add Domain → `addandknow.pt`
2. Copia os registos exactos (DKIM / SPF / MX no subdomínio `send`)
3. Define `EMAIL_FROM=Nina <no-reply@addandknow.pt>` e `RESEND_API_KEY` na Vercel
4. Redeploy
